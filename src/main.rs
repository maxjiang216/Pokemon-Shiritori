...
fn cmd_solve(opts: &HashMap<String, String>) {
    ...
    let count: usize = opts.get("count")
...
fn cmd_tournament(opts: &HashMap<String, String>) {
    let all_names = load_gen1_names();
    let count: usize = opts.get("count").and_then(|s| s.parse().ok()).unwrap_or(all_names.len());
    let depth: usize = opts.get("depth").and_then(|s| s.parse().ok()).unwrap_or(4);
    let rollouts: usize = opts.get("rollouts").and_then(|s| s.parse().ok()).unwrap_or(30);
...
fn cmd_sensitivity(opts: &HashMap<String, String>) {
    let all_names = load_gen1_names();
    let count: usize = opts.get("count").and_then(|s| s.parse().ok()).unwrap_or(all_names.len());
    let samples: usize = opts.get("samples").and_then(|s| s.parse().ok()).unwrap_or(100);
    let rollouts: usize = opts.get("rollouts").and_then(|s| s.parse().ok()).unwrap_or(40);

    let depths_str = opts.get("depths").map(|s| s.as_str()).unwrap_or("10,30,60,100,140");
    let depths: Vec<usize> = depths_str.split(',')
...
fn main() {
    ...
    match subcmd {
        ...
        _ => {
            ...
            println!();
            println!("  sensitivity  [--count N] [--depths D1,D2,...] [--samples S] [--rollouts R]");
